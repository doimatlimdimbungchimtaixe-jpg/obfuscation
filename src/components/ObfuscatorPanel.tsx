...
export function ObfuscatorPanel() {
  ...
  const optionsList = [
    { key: 'stringEncryption', label: 'String Encryption', desc: 'AES-256-CBC encryption for all string literals', icon: '🔐' },
    { key: 'controlFlow', label: 'Control Flow', desc: 'Split code into blocks with jump table dispatcher', icon: '🔀' },
    { key: 'variableRenaming', label: 'Variable Renaming', desc: 'Obfuscate local variable names to meaningless identifiers', icon: '🏷️' },
    { key: 'deadCode', label: 'Dead Code Injection', desc: 'Insert harmless dead code to confuse deobfuscators', icon: '💀' },
    { key: 'antiDebug', label: 'Anti-Debug', desc: 'Add runtime anti-debug and anti-tamper checks', icon: '🛡️' },
    { key: 'selfModifying', label: 'Self-Modifying', desc: 'Inject self-modifying code patterns', icon: '🔄' },
  ];
              ...
              {optionsList.map((opt, i) => (
                ...
                >
                  <div className="relative flex-shrink-0">
                    ...
                    />
                    <div className="w-10 h-10 rounded-xl glass flex items-center justify-center text-xl peer-checked:bg-gradient-to-br peer-checked:from-[rgb(var(--accent-primary))] peer-checked:to-[rgb(var(--accent-secondary))] peer-checked:border-[rgb(var(--accent-primary))/40] transition-all duration-300 border border-[rgb(var(--border-subtle))/50]">
                      <span className="peer-checked:hidden">{opt.icon}</span>
